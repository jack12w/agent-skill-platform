...
export default function TeamSettings({ params }: { params: { id: string } }) {
  ...
  if (error) return (<div className="max-w-4xl mx-auto px-4 sm:px-6 py-16 sm:py-24 text-center"><h1 className="text-2xl font-bold mb-2">{t('team.cannotLoad')}</h1><p className="text-gray-500 mb-6">{error}</p><Link href={`/teams/${params.id}`} className="text-blue-600 underline">{t('team.backToTeam')}</Link></div>);
  if (!team) return null;
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
      <Link href={`/teams/${params.id}`} className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-900">
