...
export default function EditSkill({ params }: { params: { slug: string } }) {
      ...
      try {
        ...
        try {
          ...
          if (pResp.ok) {
            ...
            if (pj.pricing) {
              // 兼容旧数据：member_only / both 已废弃，统一归并为 paid
              const rawMode = pj.pricing.pricing_mode || 'free';
              const mode = ['member_only', 'both'].includes(rawMode) ? 'paid' : rawMode;
              setPricing({
                pricing_mode: mode,
        ...
        // 查创作者是否有会员套餐（决定付费技能的「会员可免费下载」提示文案）
        try {
          const tType = s.owner_team_id ? 'team' : 'user';
          const tId = s.owner_team_id || s.owner_user_id;
          const cpResp = await fetch(`/api/pay/creator-plan?targetType=${tType}&targetId=${encodeURIComponent(tId)}`);
          if (cpResp.ok) {
            const cp = await cpResp.json();
            setHasMembershipPlan(!!cp?.hasPlan);
          }
        } catch {}
  ...
  const handleSave = async (e: React.FormEvent) => { e.preventDefault(); if (!skill) return; const token = localStorage.getItem('token'); if (!token) return router.push('/auth'); setSaving(true);
    try {
      const isFeatured = (skill.tags || []).includes('精选');
      const tags = ['社区', ...form.tags.split(/[,，]/).map((x) => x.trim()).filter(Boolean).filter(x => !['精选','Featured','featured','FEATURED'].includes(x))];
      ...
      try {
        ...
        if (!pr.ok) {
          ...
          alert(t('edit.pricingSaveFailed') + ': ' + (e.message || `HTTP ${pr.status}`));
        }
