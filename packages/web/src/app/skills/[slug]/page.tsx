    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
      <div className="flex flex-col md:flex-row gap-12">
        <div className="flex-1 min-w-0 md:min-w-[460px]">
          ...
          <div className="p-6 border rounded-xl">
            ...
            {versions.length === 0 ? (<p className="text-sm text-gray-500">{t('detail.noVersion')}</p>) : (
              <div className="space-y-2">{versions.map((v) => { const isLatest = skill.latest_version_id === v.id; const downloading = acting === `dl:${v.id}`; return (<div key={v.id} className="p-3 bg-gray-50 rounded-lg"><div className="flex items-center justify-between"><div className="min-w-0"><div className="flex items-center gap-2"><span className="font-mono font-medium">v{v.version}</span>{isLatest && <span className="text-xs px-2 py-0.5 bg-green-100 text-green-700 rounded">{t('detail.latest')}</span>}</div><div className="text-xs text-gray-500 mt-0.5" suppressHydrationWarning>{v.size ? `${(v.size / 1024).toFixed(1)} KB · ` : ''}{new Date(v.created_at).toLocaleString()}</div></div><button onClick={() => handleDownload(v.id)} disabled={acting !== null} className="shrink-0 text-sm px-3 py-1.5 border border-blue-600 text-blue-600 rounded hover:bg-blue-50 disabled:opacity-50 disabled:cursor-not-allowed">{downloading ? t('detail.downloading') : t('detail.download')}</button></div>{v.notes && <p className="text-xs text-gray-500 mt-1 ml-1">{v.notes}</p>}</div>); })}</div>
            )}
          </div>
        ...
        <div className="w-full md:w-80 space-y-6">
          ...
          <div className="text-sm text-gray-500"><div>{t('detail.publishedBy')}: <Link href={`/users/${encodeURIComponent(ownerName)}`} className="text-blue-600 font-medium hover:underline">{ownerName}</Link></div>{skill.owner_team && <div>{t('detail.teamLabel')}: <Link href={`/teams/${skill.owner_team.id}`} className="text-blue-600 font-medium hover:underline">{skill.owner_team.name}</Link></div>}<div>{t('detail.lastUpdated')}: <span className="text-gray-900 font-medium" suppressHydrationWarning>{skill.updated_at ? new Date(skill.updated_at).toLocaleDateString() : '-'}</span></div></div>
