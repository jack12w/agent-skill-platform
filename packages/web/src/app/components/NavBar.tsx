...
export default function NavBar() {
  ...
  const navLinks = [
    ...
    { href: '/plugins', label: t('nav.plugins'), blue: false },
  ...
  return (
    <nav className="sticky top-0 z-10 flex items-center justify-between p-4 md:p-6 border-b gap-4 bg-brand-50/60 backdrop-blur-md border-neutral-200/60">
      ...
      <div className="hidden md:flex items-center gap-4">
        <form onSubmit={handleSearch} className="w-48">
          <div className="relative group">
            <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-500 transition-colors" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t('nav.search')} className="w-full pl-9 pr-3 py-1.5 text-sm border border-neutral-200 rounded-full bg-white focus:border-brand-400 focus:outline-none transition-colors" />
            <button type="submit" className="hidden" />
          </div>
        </form>
        {navLinks.map((link) => (
          <Link key={link.href} href={link.href} className={`text-sm font-medium ${link.blue ? 'text-brand-600 hover:text-brand-800' : 'hover:text-brand-500'}`}>{link.label}</Link>
        ))}
        <div className="flex items-center gap-1 text-sm text-neutral-600 cursor-pointer hover:text-neutral-900" onClick={toggleLang}>
      ...
      <div className="flex md:hidden items-center gap-2 ml-auto">
        <button onClick={() => setShowMobileSearch(!showMobileSearch)} className="p-2 text-neutral-500 hover:text-neutral-700" aria-label="Search">
        ...
        <div className="absolute top-16 left-0 right-0 bg-white/90 backdrop-blur-md border-b shadow-lg md:hidden z-50 p-4">
          <form onSubmit={handleSearch}>
            <div className="relative">
              ...
              <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t('nav.search')} className="w-full pl-9 pr-3 py-2 text-sm border rounded-lg focus:border-brand-400 focus:outline-none" autoFocus />
              <button type="submit" className="hidden" />
            </div>
          </form>
        </div>
          ...
          <div className="flex flex-col p-4 space-y-3">
            {navLinks.map((link) => (
              <Link key={link.href} href={link.href} onClick={() => setMenuOpen(false)}
                className={`text-base font-medium py-2 ${link.blue ? 'text-brand-600' : 'hover:text-brand-500'}`}
              >
                {link.label}
              </Link>
            ))}
            ...
            <button onClick={() => { toggleLang(); setMenuOpen(false); }}
              className="flex items-center gap-2 text-base font-medium py-2 text-neutral-600 hover:text-neutral-900 w-full text-left"
            >
