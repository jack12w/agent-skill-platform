      ...
      <div className="flex gap-0 min-h-[500px] border border-gray-200 rounded-xl overflow-hidden bg-white">
        <aside className="w-48 sm:w-56 shrink-0 bg-gray-50 border-r border-gray-200">
          <div className="px-4 py-5 border-b border-gray-200">
            <h2 className="text-sm font-semibold text-gray-900">{t('help.title')}</h2>
          </div>
          <nav className="py-2">
            {MENU.map(m => (
              <button
        ...
        <main className="flex-1 p-6 sm:p-8 overflow-y-auto">
          {active === 'quickstart' && (
            <div className="prose prose-sm max-w-none">
              ...
              <p className="text-gray-500 text-sm mb-6">{t('help.qsDesc')}</p>

              <h2 className="text-lg font-semibold mt-6 mb-2">{t('help.qsStep1')}</h2>
              <p className="text-gray-600 text-sm leading-relaxed">
                {t('help.qsStep1Desc').split('技能广场')[0]}<Link href="/skills" className="text-blue-600 hover:underline">技能广场</Link>{t('help.qsStep1Desc').split('技能广场')[1] || ''}
              </p>

              <h2 className="text-lg font-semibold mt-6 mb-2">{t('help.qsStep2')}</h2>
              <p className="text-gray-600 text-sm leading-relaxed">{t('help.qsStep2Desc')}</p>

              <h2 className="text-lg font-semibold mt-6 mb-2">{t('help.qsStep3')}</h2>
              <p className="text-gray-600 text-sm leading-relaxed">
                {t('help.qsStep3Desc').split('SKILL.md')[0]}<code className="bg-gray-100 px-1 rounded">SKILL.md</code>{t('help.qsStep3Desc').split('SKILL.md')[1] || ''}
              </p>
...
(技能指令或说明内容)`}</pre>
              ...
              <p className="text-gray-600 text-sm leading-relaxed">{t('help.qsStep4Desc')}</p>
            </div>
          )}
          ...
          {active === 'about' && (
            <div className="prose prose-sm max-w-none">
              ...
              <p className="text-gray-500 text-sm mb-6">{t('help.aboutDesc')}</p>

              <h2 className="text-lg font-semibold mt-6 mb-2">{t('help.aboutPlatform')}</h2>
              <p className="text-gray-600 text-sm leading-relaxed">{t('help.aboutPlatformDesc')}</p>
              <p className="text-gray-600 text-sm leading-relaxed mt-3">{t('help.aboutPlatformDesc2')}</p>
          ...
          {active === 'contact' && (
            <div className="prose prose-sm max-w-none">
              ...
              <p className="text-gray-500 text-sm mb-6">{t('help.contactDesc')}</p>

              <h2 className="text-lg font-semibold mt-6 mb-2">{t('help.contact')}</h2>
              <div className="space-y-3 text-sm text-gray-600">
                <div className="flex items-center gap-2">
                  <span className="text-gray-400 w-16 shrink-0">{t('help.contactEmail')}</span>
                  <a href="mailto:287083583@qq.com" className="text-blue-600 hover:underline">287083583@qq.com</a>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-gray-400 w-16 shrink-0">{t('help.contactGithub')}</span>
                ...
                <div className="flex items-center gap-2">
                  <span className="text-gray-400 w-16 shrink-0">{t('help.contactWebsite')}</span>
          ...
          {active === 'feedback' && (
            <div className="prose prose-sm max-w-none">
              ...
              <p className="text-gray-500 text-sm mb-4">{t('help.fbDesc')}</p>

              {typeof window !== 'undefined' && !localStorage.getItem('token') ? (
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 text-sm text-amber-700">
                  {t('help.fbLoginHint').split('登录')[0]}<a href="/auth" className="text-blue-600 hover:underline font-medium">登录</a>{t('help.fbLoginHint').split('登录')[1] || ''}
                </div>
              ) : fbSent ? (
                ...
                <form onSubmit={submitFeedback} className="space-y-4 max-w-lg">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">{t('help.fbName')} *</label>
                    <input type="text" required value={fbName} onChange={e => setFbName(e.target.value)} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:border-blue-400" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">{t('help.fbEmail')}</label>
                  ...
                  <div>
                    ...
                    <textarea required value={fbContent} onChange={e => setFbContent(e.target.value)} rows={5} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:border-blue-400 resize-y" placeholder={t('help.fbPlaceholder')} />
                  </div>
