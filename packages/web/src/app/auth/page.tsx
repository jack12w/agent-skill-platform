...
export default function AuthPage() {
  ...
  // 手机微信内：走公众号静默登录（snsapi_base），不走 PC 扫码弹窗
  const handleMpLogin = () => {
    window.location.href =
      '/api/auth/wechat/mp/url?redirect=' + encodeURIComponent(redirect || '/dashboard');
  };
    ...
    <div className="max-w-md mx-auto px-4 sm:px-6 py-16 sm:py-24">
      ...
      {process.env.NEXT_PUBLIC_WECHAT_LOGIN_ENABLED === 'true' && (
        <div className="mt-6">
          ...
          <button onClick={isWechat() ? handleMpLogin : handleWechatLogin} className="w-full py-3 bg-green-500 text-white rounded-lg font-bold hover:bg-green-600 flex items-center justify-center gap-2">
