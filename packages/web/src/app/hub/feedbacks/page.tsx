...
export default function HubFeedbacksPage() {
  ...
  const typeColor = (v: string) => v === 'bug' ? 'bg-red-50 text-red-600' : v === 'suggestion' ? 'bg-blue-50 text-blue-600' : 'bg-gray-100 text-gray-600';
  ...
  return (
    <div>
      <h1 className="text-xl font-bold text-gray-900 mb-1">{t('help.fbAdminTitle')}</h1>
      <p className="text-xs text-gray-500 mb-4">{t('help.fbAdminDesc')}</p>
            ...
            {(data?.items || []).map((f: any) => (
              <tr key={f.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-right">
                  <button onClick={() => deleteFeedback(f.id)} className="text-xs text-red-500 hover:underline">{t('help.fbAdminDelete')}</button>
                </td>
              </tr>
            ))}
            {(!data || data.items.length === 0) && (
              <tr><td colSpan={6} className="px-4 py-12 text-center text-gray-400 text-sm">{t('help.fbAdminEmpty')}</td></tr>
            )}
          </tbody>
        </table>
      </div>
