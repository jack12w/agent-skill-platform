...
// 日期格式化为 YYYY-MM-DD；空值返回 null 由调用方决定占位
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 my-4 flex-wrap">
        ...
        {data?.activeUsers && (
          <span className="px-2 py-1 bg-white border border-neutral-200 rounded-lg text-xs text-neutral-500 whitespace-nowrap">
            {t('admin.dau')} <span className="font-semibold text-brand-700">{data.activeUsers.dau}</span>
          </span>
        )}
        ...
        {data?.activeUsers && (
          <div className="flex items-center gap-1.5 flex-wrap">
            {([
              { k: 'd7', label: t('admin.active7d') },
              { k: 'd30', label: t('admin.active30d') },
              { k: 'd90', label: t('admin.active90d') },
              { k: 'd180', label: t('admin.active180d') },
              { k: 'd365', label: t('admin.active365d') },
            ] as const).map(x => (
              <span key={x.k} className="px-2 py-1 bg-white border border-neutral-200 rounded-lg text-xs text-neutral-500 whitespace-nowrap">
                {x.label} <span className="font-semibold text-brand-700">{data.activeUsers[x.k]}</span>
              </span>
            ))}
          </div>
        )}
          ...
          <tbody className="divide-y divide-neutral-100">
            {(data?.items || []).map((u: any) => (
              <tr key={u.id} className="hover:bg-neutral-100">
                ...
                <td className="px-4 py-3 text-neutral-500 whitespace-nowrap">{fmtDate(u.created_at) || '-'}</td>
                <td className="px-4 py-3 whitespace-nowrap">
                  {u.last_seen_at
                    ? <span className="text-neutral-700">{fmtDate(u.last_seen_at)}</span>
                    : <span className="text-neutral-400">{t('admin.neverActive')}</span>}
                </td>
