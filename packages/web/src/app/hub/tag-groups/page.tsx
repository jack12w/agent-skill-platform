...
export default function HubTagGroupsPage() {
  ...
  return (
    <div>
      <h1 className="text-xl font-bold text-gray-900 mb-1">标签分组管理</h1>
      <p className="text-xs text-gray-500 mb-4">修改后前端的技能广场、编辑页、提交页标签同步生效</p>
      {msg && <p className="text-sm text-green-600 mb-2">{msg}</p>}

      {/* Add new */}
      <div className="bg-white border rounded-xl p-4 mb-4">
        <h2 className="text-sm font-semibold mb-3">新增分组</h2>
        <div className="flex flex-wrap gap-3 items-end">
          <div>
            <label className="text-xs text-gray-500">Key (英文标识)</label>
          ...
          <div>
            ...
            <input value={newGroup.name} onChange={e => setNewGroup({ ...newGroup, name: e.target.value })} className="w-24 px-2 py-1.5 text-sm border rounded" placeholder="角色" />
          </div>
          <div>
            <label className="text-xs text-gray-500">标签（逗号分隔）</label>
            <input value={newGroup.tagsStr} onChange={e => setNewGroup({ ...newGroup, tagsStr: e.target.value })} className="w-64 px-2 py-1.5 text-sm border rounded" placeholder="标签1, 标签2, 标签3" />
          </div>
          <button onClick={create} className="px-4 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">添加</button>
        </div>
      </div>
        ...
        <div key={g.id} className="bg-white border rounded-xl p-4 mb-3">
          {editing?.id === g.id ? (
            <div className="flex flex-wrap gap-3 items-end">
              ...
              <button onClick={update} className="px-3 py-1.5 text-sm bg-green-600 text-white rounded">保存</button>
              <button onClick={() => setEditing(null)} className="px-3 py-1.5 text-sm bg-gray-200 rounded">取消</button>
            </div>
          ) : (
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs text-gray-400">({g.tags?.length || 0} 个标签)</span>
                </div>
              ...
              <div className="flex gap-2 shrink-0 ml-4">
                <button onClick={() => setEditing({ id: g.id, name: g.name, tagsStr: (g.tags || []).join(', ') })} className="text-xs text-blue-600 hover:underline">编辑</button>
                <button onClick={() => del(g.id)} className="text-xs text-red-500 hover:underline">删除</button>
              </div>
            </div>
          )}
        </div>
