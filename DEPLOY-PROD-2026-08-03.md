...
> 适用范围：将 2026-08-02 ~ 2026-08-05 的全部更新部署到生产环境（skills.rehomi.com）。
> 本次部署包含：微信支付系统、创作者会员制、支付安全审计修复、**支付资金安全多轮加固**
> （提现双扣/误解冻/余额双入账/退款少扣/会员续订撞键/冻结非原子竞态等）、定价防呆、
> 管理后台修复（折线图/底部通栏/回调日志）、用户活跃统计。
> 对应代码版本：main 分支 `53d955a`（含）及之前全部提交。
...
预期最新提交（`git log --oneline -3` 应看到这些，HEAD 须为 `53d955a`）：

```
53d955a fix(payments): freeze 改为原子条件递减 + 清理死代码 + 转账回调头兼容
8e1df03 refactor(payments): 优化 4 项低风险项（事务化提现、并发重试、通道一致性、清理死代码）
e22483b fix(payments): 修复提现 freeze 失败时的误解冻（资金安全 HIGH）
```
